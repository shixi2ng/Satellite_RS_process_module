    ...
    for hei_limit in range(5, 6):
        for mode in range(3, 6):
            ...
            index_list2 = ['S2_OSAVI_20m_noninun_linear_interpolation',
                           ...
                           'S2_B4_noninun_linear_interpolation', 'S2_B3_noninun_linear_interpolation',
                           'S2_B2_noninun_linear_interpolation', ]
            index_list3 = ['S2_OSAVI_20m_noninun_linear_interpolation', 'S2_MNDWI_linear_interpolation', 'S2_NDVI_20m_noninun_linear_interpolation',
                           'S2_B8A_noninun_linear_interpolation', 'S2_B9_noninun_linear_interpolation',
                           'S2_B7_noninun_linear_interpolation', 'S2_B6_noninun_linear_interpolation',
                           'S2_B5_noninun_linear_interpolation',
                           'S2_B4_noninun_linear_interpolation', 'S2_B3_noninun_linear_interpolation',
            ...
            gedi_df = gedi_df.loc[gedi_df['Canopy Height (rh100)'] >= 2.2]
            ...
            if cross_validation_factor:
                for subsample in range(10, 21, 1):
                    subsample = subsample / 20
                    # fig4, ax4 = plt.subplots(figsize=(12, 8), constrained_layout=True)
                    for max_d in range(19, 22):
                        # train_all = []
                        # test_all = []
                        for n_est in range(500, 600, 50):
                            ...
                            for year in [2019, 2020, 2021, 2022]:
                                ...
                                print('----------------------------------------------------------------------------------------------')
                                print(f'Validation year:{str(year)}, Mode:{str(mode)}, Max depth:{str(21)}, N_est:{str(300)}')
                            ...
                            if not os.path.exists(f'G:\\A_veg\\S2_all\\XGB_vhm\\Model_acc_CrossVal\\XGB_VHM_Mod{str(mode)}_Nest{str(n_est)}_maxd{str(max_d)}_sub{str(int(subsample * 100))}per.csv'):
                                acc_df.to_csv(f'G:\\A_veg\\S2_all\\XGB_vhm\\Model_acc_CrossVal\\XGB_VHM_Mod{str(mode)}_Nest{str(n_est)}_maxd{str(max_d)}_sub{str(int(subsample * 100))}per.csv')
