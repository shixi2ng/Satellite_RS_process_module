...
DownPath = 'E:\\A_Vegetation_Identification\\Wuhan_Sentinel_L2_Original\\supplement\\'

api = SentinelAPI('shixi2ng', 'shixi2nG', 'https://scihub.copernicus.eu/dhus')

url_file = 'E:\\A_Vegetation_Identification\\Wuhan_Sentinel_L2_Original\\supplement\\products.meta4'
...
while linklist:
    ...
    if product_info['Online']:
        ...
        call([IDM, '/d', link, '/p', DownPath, '/n', '/a'])  # 将连接添加到任务。/a表示先不下载
        linklist.remove(link)
        call([IDM, '/s'])  # 开启下载
        print("Start download")

BLUE_temp_band = BLUE_temp_ds.GetRasterBand(1)
BLUE_temp_array = gdal_array.BandReadAsArray(BLUE_temp_band)
BLUE_temp_array[BLUE_temp_array == BLUE_temp_band.GetNoDataValue()] = np.nan
BLUE_temp_array = BLUE_temp_array.astype(np.float16) * 0.0000275 - 0.2