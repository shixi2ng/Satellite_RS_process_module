...
class ISCCP_PAR(object):
        ...
        for file in self.nc_files:
            try:
                ...
                if 'ISCCP_HXG_total_PAR' in file:
                    ...
                    hour = filename.split('_')[7].split('.')[0]
    ...
    def _nc2tif(self, doy: int, crs: str = None):
        ...
        try:
            s_t = time.time()
            print(f'Start generating the\033[1;31m Dpar \033[0mat the DOY \033[1;34m{str(doy)}\033[0m')

            # Create output folder
            ori_tif_output_folder = f'{self.output_path}Ori_Denv_raster\\DPAR\\'
            bf.create_folder(ori_tif_output_folder)

            if not os.path.exists(f"{ori_tif_output_folder}{str(doy)}_DPAR.TIF"):
                # Process the nc file
                files = list(self.metadata_df[self.metadata_df['DOY'] == doy]['FileDir'])

                # Define WGS84 projection and proper geotransform
                srs = osr.SpatialReference()
                srs.ImportFromEPSG(4326)  # WGS84
                projection = srs.ExportToWkt()

                # Calculate geotransform for global coverage
                # Format: (top_left_x, pixel_width, rotation, top_left_y, rotation, pixel_height)
                geotransform = (-180.0, 0.1, 0,  # 360/3600 = 0.1 degree per pixel
                                90.0, 0, -0.1)  # 180/1800 = 0.1 degree per pixel
                ...
                for file in files:
                    ...
                    ncfile_list.append(ds_temp.GetRasterBand(1).ReadAsArray())
                    ds_temp = None

                # Calculate the average
                mean_arr = np.mean(np.dstack(ncfile_list), axis=2)

                # Create output GeoTIFF
                driver = gdal.GetDriverByName('GTiff')
                dst_ds = driver.Create(
                    f"{ori_tif_output_folder}{str(doy)}_DPAR.TIF",
                    3600,  # width
                    1800,  # height
                    1,  # number of bands
                    gdal.GDT_Float32
                ...
                # Set arr
                dst_ds.GetRasterBand(1).WriteArray(mean_arr)
                # Set nodata
            ...
            print( f'Finish generating the\033[1;31m Dpar \033[0mat the DOY \033[1;34m{str(doy)}\033[0m in \033[1;31m{str(time.time() - s_t)}\033[0ms')
        except:
            print(traceback.format_exc())
            raise Exception(f'Error in processing the nc file of {str(doy)}!')
    ...
    def _extract_ras_with_ROI(self, file: str, ROI, bounds: list = None, ras_res: list = None):
        ...
        if self.ROI is not None:
            ...
            if not os.path.exists(f"{self.output_path}{self.ROI_name}_Denv_raster\\DPAR\\{filename}"):
                ...
                # Get roi proj
                roi_ds = ogr.Open(ROI)
                roi_proj = roi_ds.GetLayer().GetSpatialRef().GetAuthorityCode(None)

                # Get raster proj
                ds_temp = gdal.Open(file)
                proj_wkt = ds_temp.GetProjection()
                spatial_ref = osr.SpatialReference()
                spatial_ref.ImportFromWkt(proj_wkt)
                ras_proj = spatial_ref.GetAuthorityCode(None)

                if ras_proj != roi_proj:
                    dst_proj = osr.SpatialReference()
                    dst_proj.ImportFromEPSG(int(roi_proj))
                    gdal.Warp(f"/vsimem/{self.ROI_name}{filename.split('.TIF')[0]}_convert.TIF", ds_temp, dstSRS=dst_proj)
                    gdal.Warp(f"/vsimem/{self.ROI_name}{filename.split('.TIF')[0]}_TEMP.TIF", f"/vsimem/{self.ROI_name}{filename.split('.TIF')[0]}_convert.TIF",
                              xRes=ras_res[0], yRes=ras_res[1], resampleAlg=gdal.GRA_CubicSpline, outputBounds=bounds)
                else:
                    gdal.Warp(f"/vsimem/{self.ROI_name}{filename.split('.TIF')[0]}_TEMP.TIF", ds_temp,
                              xRes=ras_res[0], yRes=ras_res[1], resampleAlg=gdal.GRA_CubicSpline, outputBounds=bounds)
                gdal.Warp(f"/vsimem/{self.ROI_name}{filename.split('.TIF')[0]}_TEMP2.TIF",
                          f"/vsimem/{self.ROI_name}{filename.split('.TIF')[0]}_TEMP.TIF", xRes=ras_res[0],
                          yRes=ras_res[1], cutlineDSName=self.ROI, outputBounds=bounds)
                gdal.Translate(f"{self.output_path}{self.ROI_name}_Denv_raster\\DPAR\\{filename}",
                               f"/vsimem/{self.ROI_name}{filename.split('.TIF')[0]}_TEMP2.TIF", options=topts)
                if ras_proj != roi_proj:
                    gdal.Unlink(f"/vsimem/{self.ROI_name}{filename.split('.TIF')[0]}_convert.TIF")
    ...
    def raster2dc(self, index_list: list, temporal_division=None, ROI=None, **kwargs):
        ...
        for index_temp in index_list:
            ...
            metadata_dic = {'ROI_name': self.ROI_name, 'index': index_temp, 'Datatype': 'float', 'ROI': self.ROI,
                            ...
                            'Denv_factor': True, 'Nodata_value': 0,
            ...
            with concurrent.futures.ProcessPoolExecutor(max_workers=2) as executor:
                executor.map(self._raster2dc, repeat(output_path), repeat(input_folder), time_range, repeat(index_temp),
                             repeat(metadata_dic), repeat(rows), repeat(cols), repeat(_sparse_matrix))

    def _raster2dc(self, output_path, input_folder, time_temp, zvalue_temp, metadata_dic, rows, cols, _sparse_matrix, ):
        ...
        if not os.path.exists(f'{yearly_output_path}doy.npy') or not os.path.exists(
                f'{yearly_output_path}metadata.json'):

            # Determine the input files
            yearly_input_files = bf.file_filter(input_folder, ['.TIF', '\\' + str(time_temp)],
                                                exclude_word_list=['aux'], and_or_factor='and')
    ...
    def _cal_dailyPAR(self, doy: int, method='mean', remove_low_quality_data=True) -> None:
        ...
        try:
            if not os.path.exists(f'{output_path}{str(doy)}_DPAR.TIF'):
                s_t = time.time()
                print(f'Start generating the\033[1;31m Dpar \033[0mat the DOY \033[1;34m{str(doy)}\033[0m')
                file_list = bf.file_filter(input_path, [str(doy), 'GMT'], and_or_factor='and', exclude_word_list=['.aux', '.ovr'])
                if file_list == []:
                ...
                for file in file_list:
                    ...
                    # Remove invalid data
                    array_temp[array_temp == nodatavalue] = np.nan
                    array_temp[array_temp < 0] = np.nan
                    array_list.append(array_temp)

                # Calculate the average
                if method == 'mean':
                    stacked_arrays = np.dstack(array_list)
                    array_temp_ave = np.nanmean(stacked_arrays, axis=2)
                else:
                    pass

                # Output the daily par tif file
                bf.write_raster(ds_temp, array_temp, output_path, f'{str(doy)}_DPAR.TIF', raster_datatype=gdal.GDT_Float32, nodatavalue=np.nan)
                print(f'Finish generating the\033[1;31m Dpar \033[0mat the DOY \033[1;34m{str(doy)}\033[0m in \033[1;31m{str(time.time() - s_t)}\033[0ms')
                ds_temp, array_listm, stacked_arrays = None, None, None
        except:
            print(traceback.format_exc())
            print('Error in the daily PAR calculation!')
    ...
    def raster2dc(self, index_list: list, temporal_division=None, ROI=None, ** kwargs):
        ...
        for index_temp in index_list:
            ...
            metadata_dic = {'ROI_name': self.ROI_name, 'index': index_temp, 'Datatype': 'float', 'ROI': self.ROI,
                            ...
                            'coordinate_system': self.main_coordinate_system, 'size_control_factor': False, 'Nodata_value': 0,
            ...
            # Sequence process the raster into dc
            for _ in time_range:
                self._raster2dc(output_path, input_folder, _, index_temp, metadata_dic, rows, cols, _sparse_matrix)
            # with concurrent.futures.ProcessPoolExecutor(max_workers=2) as executor:
            #     executor.map(self._raster2dc, repeat(output_path), repeat(input_folder), time_range, repeat(index_temp),
            #                  repeat(metadata_dic), repeat(rows), repeat(cols), repeat(_sparse_matrix))
    ...
    def _raster2dc(self, output_path, input_folder, time_temp, zvalue_temp, metadata_dic, rows, cols, _sparse_matrix, ):
        ...
        try:
            ...
            if not os.path.exists(f'{yearly_output_path}doy.npy') or not os.path.exists(f'{yearly_output_path}metadata.json'):
                ...
                # Determine the input files
                yearly_input_files = bf.file_filter(input_folder, ['.TIF', '\\' + str(time_temp)], exclude_word_list=['aux', 'ovr'], and_or_factor='and')
                if yearly_input_files == []:
                    raise Exception('There are no valid input files, double check the temporal division!')
                ...
                doy_list = [int(filepath_temp.split('\\')[-1][0:7]) for filepath_temp in yearly_input_files]
                # Check doy list
                doy_list = np.sort(np.unique(np.array(doy_list))).tolist()

                # Determine whether the output folder is huge and sparsify or not?
                mem = psutil.virtual_memory()
                dc_max_size = int(mem.free * 0.90)
                _huge_matrix = True if len(doy_list) * cols * rows * 8 > dc_max_size else False
                ...
                if _huge_matrix:
                    if _sparse_matrix:
                        ...
                        metadata_dic['Datatype'] = str(np.uint16)
                        np.save(f'{yearly_output_path}doy.npy', doy_list)
                        data_cube.save(f'{yearly_output_path}{zvalue_temp}_Denv_datacube\\')
                    else:
                        pass
            ...
            print(traceback.format_exc())
            raise Exception('During the raster2dc process, the above error occurred!')


if __name__ == '__main__':

    # for _ in range(1984, 2001):
    #     par_isccp = ISCCP_PAR(f'G:\\A_Climatology_dataset\\gridded_dataset\\ISCCP-PAR\\{str(_)}\\')
    #     # bounds_temp = bf.raster_ds2bounds('G:\\A_Climatology_dataset\\gridded_dataset\\MODIS_PAR_V6.2\\ROI_map\\floodplain_2020.TIF')
    #     # par_isccp.mp_extract_with_ROI('G:\\A_Landsat_Floodplain_veg\\ROI_map\\floodplain_2020_UTM.shp', ['DPAR'], bounds=bounds_temp, ras_res=[30, 30])
    #     par_isccp.raster2dc(['DPAR'], ROI='G:\\A_Landsat_Floodplain_veg\\ROI_map\\floodplain_2020.shp', temporal_division='year', inherit_from_logfile=True)
