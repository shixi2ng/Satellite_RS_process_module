...
if __name__ == '__main__':
    ...
    # s2_ds_temp.mp_mosaic2seqtif(['B8', 'B8A', 'B9'], inherit_from_logfile=True)
    # while True:
    #     try:
    #         s2_ds_temp.mp_ds2sdc(['B2', 'B3', 'B4', 'B5', 'B6', 'B7', 'B8', 'B8A', 'B9', 'OSAVI_20m', 'NDVI_20m', 'MNDWI'], inherit_from_logfile=True, remove_nan_layer=True, chunk_size=3)
    #         break
    #     except:
    #         pass
    ...
    # for index in ['B2', 'B3', 'B4', 'B5', 'B6', 'B7', 'B8', 'B8A', 'B9', 'OSAVI_20m', 'NDVI_20m']:
    ...
    for index_temp in [['B2_noninun'], ['B3_noninun'], ['B4_noninun'], ['B5_noninun'], ['B6_noninun'], ['B7_noninun'], ['B8_noninun'], ['B8A_noninun'], ['B9_noninun'], ['OSAVI_20m_noninun'], ['MNDWI'], ['NDVI_20m_noninun']]:
        list = []
        for index in index_temp:
            list.append(Sentinel2_dc(f'G:\\A_veg\\S2_all\\Sentinel2_L2A_Output\\Sentinel2_MYZR_FP_2020_datacube\\{index}_sequenced_datacube\\'))
        dcs_temp = Sentinel2_dcs(list[0])
        dcs_temp.link_GEDI_S2_inform('G:\A_veg\S2_all\GEDI_V2\\floodplain_2020_high_quality.xlsx', index_temp, retrieval_method='linear_interpolation')
