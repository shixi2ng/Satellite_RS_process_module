...
if __name__ == '__main__':
    ...
    # for denv_, thr_ in zip(['TEM', 'RHU', 'PRS', 'PRE', 'WIN'], [['static_thr', 1000.0], ['static_thr', 0.0], ['static_thr', 0.0], ['static_thr', 0.0], ['static_thr', 0.0]]):
    #     dc_temp_dic = []
    #     for year in [str(_) for _ in range(2018, 2024)]:
    ...
    #         dc_temp_dic.append(Phemetric_dc(f'G:\\A_Landsat_Floodplain_veg\\Landsat_floodplain_2020_datacube\\OSAVI_noninun_curfit_datacube\\floodplain_2020_Phemetric_datacube\\{year}\\'))
    #     rsdc_temp = RS_dcs(*dc_temp_dic)
    #     rsdc_temp.link_GEDI_Denvdc(GEDI_df('G:\\A_GEDI_Floodplain_vegh\\GEDI_MYR\\L2_vegh\\floodplain_2020_high_quality.xlsx'), [denv_], spatial_interpolate_method=['area_average', 'focal'], accumulated_period=['SOY2DOY', 'SOY2PEAK'], threshold = thr_)

    # # Link high quality GEDI data with PAR
    # for denv_, thr_ in zip(['DPAR'], [['static_thr', 0.0]]):
    #     dc_temp_dic = []
    #     for year in [str(_) for _ in range(2018, 2024)]:
    ...
    # Link high quality GEDI data with Phedc
    for year in [str(_) for _ in range(1992, 2024)]:
        dc_temp_dic = []
        dc_temp_dic.append(Phemetric_dc(f'G:\\A_Landsat_Floodplain_veg\\Landsat_floodplain_2020_datacube\\OSAVI_noninun_curfit_datacube\\floodplain_2020_Phemetric_datacube\\{year}\\'))
        rsdc_temp = RS_dcs(*dc_temp_dic)
        rsdc_temp.link_GEDI_Phemedc(GEDI_df(f'G:\\A_GEDI_Floodplain_vegh\\GEDI_MYR\\L2_vegh\\simulated_GEDI_df\\peak_{year}_simulated.csv'), ['DR', 'DR2', 'EOS', 'GR', 'SOS', 'peak_vi', 'peak_doy', 'trough_vi', 'MAVI'], spatial_interpolate_method=['area_average', 'focal'],)
    ...
    for index_ in index_list:
        ...
        rs_dc_temp = RS_dcs(*dc_temp_dic)
        for year in [str(_) for _ in range(1987, 2024)]:
            rs_dc_temp.link_GEDI_RS_dc(GEDI_df(f'G:\\A_GEDI_Floodplain_vegh\\GEDI_MYR\\L2_vegh\\simulated_GEDI_df\\peak_{year}_simulated.csv'), [index_], spatial_interpolate_method=['area_average', 'focal'], temporal_interpolate_method = ['linear_interpolation', '24days_max', '24days_ave'])

    # Link high quality GEDI data with Denv dc
    for year in [str(_) for _ in range(2022, 2024)]:
        dc_temp_dic, rsdc_temp = [], []
        for denv_, thr_ in zip(['TEM', 'RHU', 'PRS', 'PRE', 'WIN'], [['static_thr', 1000.0], ['static_thr', 0.0], ['static_thr', 0.0], ['static_thr', 0.0], ['static_thr', 0.0]]):
            dc_temp_dic.append(Denv_dc(f'G:\\A_Climatology_dataset\\station_dataset\\CMA_dataset\\2400_all_station_1950_2023\\CMA_OUTPUT\\floodplain_2020_UTM_Denv_datacube\\{denv_}\\{year}\\'))
        dc_temp_dic.append(Phemetric_dc(f'G:\\A_Landsat_Floodplain_veg\\Landsat_floodplain_2020_datacube\\OSAVI_noninun_curfit_datacube\\floodplain_2020_Phemetric_datacube\\{year}\\'))
        rsdc_temp = RS_dcs(*dc_temp_dic)
        rsdc_temp.link_GEDI_Denvdc(GEDI_df(f'G:\\A_GEDI_Floodplain_vegh\\GEDI_MYR\\L2_vegh\\simulated_GEDI_df\\peak_{year}_simulated.csv'), ['TEM', 'RHU', 'PRS', 'PRE', 'WIN'], spatial_interpolate_method=['area_average', 'focal'], accumulated_period=['SOY2DOY', 'SOY2PEAK'], threshold = thr_)
    ...
    #         dc_temp_dic.append(Phemetric_dc(f'G:\\A_Landsat_Floodplain_veg\\Landsat_floodplain_2020_datacube\\OSAVI_noninun_curfit_datacube\\floodplain_2020_Phemetric_datacube\\{year}\\'))
    #     rsdc_temp = RS_dcs(*dc_temp_dic)
    #     rsdc_temp.link_GEDI_Denvdc(GEDI_df(f'G:\\A_GEDI_Floodplain_vegh\\GEDI_MYR\\L2_vegh\\simulated_GEDI_df\\peak_{year}_simulated.csv'), [denv_], spatial_interpolate_method=['area_average', 'focal'], accumulated_period=['SOY2DOY', 'SOY2PEAK'], threshold = thr_)
    #
    # # Link high quality GEDI data with VegType
    # for year in [str(_) for _ in range(2018, 2024)]:
    #     rsdc_temp = RS_dcs(Phemetric_dc(f'G:\\A_Landsat_Floodplain_veg\\Landsat_floodplain_2020_datacube\\OSAVI_noninun_curfit_datacube\\floodplain_2020_Phemetric_datacube\\2018\\'))
    #     rsdc_temp.link_GEDI_VegType('G:\\A_GEDI_Floodplain_vegh\\Veg_map\\results\\Classification\\veg_map_ex_v2.tif', GEDI_df(f'G:\\A_GEDI_Floodplain_vegh\\GEDI_MYR\\L2_vegh\\simulated_GEDI_df\\peak_{year}_simulated.csv'), spatial_interpolate_method=['area_average', 'focal'])
    # bf.merge_csv_files("G:\\A_GEDI_Floodplain_vegh\\GEDI_MYR\\L2_vegh\\GEDI_link_RS\\", 'G:\\A_GEDI_Floodplain_vegh\\GEDI_MYR\\L2_vegh\\floodplain_GEDI_2019_2023_for_model_high_quality.csv', merge_keys = ['Quality Flag', 'Canopy Elevation (m)', 'index', 'EPSG_lat', 'Elevation (m)', 'Beam', 'Landsat water rate', 'Latitude', 'Shot Number', 'Sensitivity', 'EPSG_lon', 'Tandem-X DEM', 'Longitude', 'Degrade Flag', 'Date', 'RH 98', 'RH 25', 'Urban rate', 'Canopy Height (rh100)', 'Leaf off flag'])
    #
    # # TRAIN VHM
    # VHM = VHM('XGB')
