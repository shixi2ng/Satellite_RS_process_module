...
class built_in_index(object):
    ...
    def __init__(self, *args):
        ...
        self.SVVI = "SVVI = (sqrt(((BLUE - ((BLUE + GREEN + RED + NIR + SWIR + SWIR2) / 6)) ** 2 + (GREEN - ((BLUE + GREEN + RED + NIR + SWIR + SWIR2) / 6)) ** 2 + (RED - ((BLUE + GREEN + RED + NIR + SWIR + SWIR2) / 6)) ** 2 + (NIR - ((BLUE + GREEN + RED + NIR + SWIR + SWIR2) / 6)) ** 2 + (SWIR - ((BLUE + GREEN + RED + NIR + SWIR + SWIR2) / 6)) ** 2 + (SWIR2 - ((BLUE + GREEN + RED + NIR + SWIR + SWIR2) / 6)) ** 2) / 6) - sqrt(((NIR - ((NIR + SWIR + SWIR2) / 3)) ** 2 + (SWIR - ((NIR + SWIR + SWIR2) / 3)) ** 2 + (SWIR2 - ((NIR + SWIR + SWIR2) / 3)) ** 2) / 3))"
        self.TCGREENESS = 'TCGREENESS = (- 0.1603 * BLUE - 0.2819 * GREEN - 0.4934 * RED + 0.7940 * NIR - 0.0002 * SWIR - 0.1446 * SWIR2)'
        self.BLUE = 'BLUE = BLUE'
